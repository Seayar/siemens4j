package xyz.seayar.siemens4j;

import xyz.seayar.siemens4j.basic.SoftBasic;
import xyz.seayar.siemens4j.core.imessage.S7Message;
import xyz.seayar.siemens4j.core.net.networkbase.NetworkDeviceBase;
import xyz.seayar.siemens4j.core.transfer.ReverseBytesTransform;
import xyz.seayar.siemens4j.core.types.OperateResult;
import xyz.seayar.siemens4j.core.types.OperateResultExOne;
import xyz.seayar.siemens4j.core.types.OperateResultExThree;
import xyz.seayar.siemens4j.message.MessageString;
...
public class SiemensS7Net extends NetworkDeviceBase<S7Message, ReverseBytesTransform> {
    ...
    private OperateResultExOne<byte[]> ReadBitFromPLC(String address) {
        ...
        if (read.Content.length >= 21 && read.Content[20] == 1) {
            ...
            return new OperateResultExOne<byte[]>(read.ErrorCode, MessageString.SiemensDataLengthCheckFailed());
        }
    }
    ...
    private OperateResult WriteBase(byte[] entireValue) {
        ...
        if (write.Content[write.Content.length - 1] != (byte) 0xFF) {
            return new OperateResult(write.Content[write.Content.length - 1], MessageString.SiemensWriteError() + write.Content[write.Content.length - 1]);
        } else {
            return OperateResult.CreateSuccessResult();
        }
    }
    ...
    private final byte[] plcOrderNumber = new byte[]
            {
    ...
    private final byte[] plcHead1_200smart = new byte[]
            {
    ...
    private final byte[] plcHead2_200smart = new byte[]
            {
    ...
    private final byte[] plcHead1_200 = new byte[]
            {
    ...
    private final byte[] plcHead2_200 = new byte[]
            {
                    0x03, 0x00, 0x00, 0x19, 0x02, (byte) 0xF0, (byte) 0x80, 0x32, 0x01, 0x00, 0x00, (byte) 0xCC, (byte) 0xC1,
                    0x00, 0x08, 0x00, 0x00, (byte) 0xF0, 0x00, 0x00, 0x01, 0x00, 0x01, 0x03, (byte) 0xC0
            };
    ...
    private OperateResultExThree<Byte, Integer, Integer> AnalysisAddress(String address) {
        ...
        try {
            ...
            if (address.charAt(0) == 'P') {
                ...
                result.Content2 = CalculateAddressStarted(address.substring(1));
            } else if (address.charAt(0) == 'D' || "DB".equals(address.substring(0, 2))) {
    ...
    public static OperateResultExOne<byte[]> BuildReadCommand(OperateResultExThree<Byte, Integer, Integer>[] address, short[] length) {
        ...
        if (address.length != length.length) {
            throw new RuntimeException(MessageString.TwoParametersLengthIsNotSame());
        }
        if (length.length > 19) {
            throw new RuntimeException(MessageString.SiemensReadLengthCannotLargerThan19());
        }
